...
export async function listPostsSimple(page = 1, size = 10): Promise<Post[]> {
  ...
  (imgs ?? [])
    ...
    .forEach((row) => {
      if (!latestPathByPost.has(row.post_id)) latestPathByPost.set(row.post_id, row.path);
    });
...
export async function getPostSimple(id: string): Promise<Post> {
  ...
  const imageUrls =
    images?.map((i) => supabase.storage.from('post-images').getPublicUrl(i.path).data.publicUrl) ?? [];
  ...
  if (commentsRows?.length) {
    const cAuthorIds = Array.from(new Set(commentsRows.map((c) => c.author_id)));
...
export async function createPost(params: { title: string; content: string; files?: Blob[] }) {
  const {
    data: { user },
  ...
  const { data: row, error: ierr } = await supabase
    .from('posts')
    .insert({ title: params.title, content: params.content, author_id: user.id })
    .select('id,title,content,author_id,created_at,updated_at')
    .single();
  if (ierr) throw ierr;
  ...
  if (params.files?.length) {
    ...
    for (const blob of params.files) {
      ...
      const up = await supabase.storage.from('post-images').upload(path, blob, { upsert: false });
      if (up.error) throw up.error;
      paths.push(path);
    }
    const rows = paths.map((path) => ({ post_id: base.id, author_id: user.id, path }));
